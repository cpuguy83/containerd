...
func doRawCopy(rsc, wsc syscall.RawConn, remain int64) (handled bool, written int64, spliceErr error) {
	var (
		writeErr error
	)
	...
	err := rsc.Read(func(rfd uintptr) bool {
		err := wsc.Write(func(wfd uintptr) bool {
			...
			for spliceErr == nil {
				...
				n, spliceErr = unix.Splice(int(rfd), nil, int(wfd), nil, int(remain), unix.SPLICE_F_MOVE|unix.SPLICE_F_NONBLOCK)
				if n > 0 {
					written += n
					handled = true
				}
